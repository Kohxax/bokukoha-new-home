...
<template>
  <Card class="w-full bg-card backdrop-blur top-24 sticky max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
            ...
            <ul v-if="link.children && link.children.length > 0" class="mt-2 space-y-2 pl-2">
              <li v-for="(childLink, j) in link.children" :key="childLink.id">
                ...
                >
                 ...
                 <ul v-if="childLink.children && childLink.children.length > 0" class="mt-1 space-y-1 pl-2">
                    <li v-for="(grandChildLink, k) in childLink.children" :key="grandChildLink.id">
                        ...
                        >
                          <span 
                            ...
                            :class="activeId === grandChildLink.id ? 'text-primary' : 'text-muted-foreground/60'"
                          >
                            {{ i + 1 }}.{{ j + 1 }}.{{ k + 1 }}.
                          </span>
