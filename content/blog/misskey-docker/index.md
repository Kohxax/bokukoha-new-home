...
tags:
  - Misskey
  - Docker
description: タイトル通りです。言っちゃえばMisskey公式ドキュメントの「Docker Composeを使ったMisskey構築」のページをなぞりつつ、僕が詰まった箇所を少し解説するだけなので、公式ドキュメントを読める人はこの記事を読む必要はないと思います。自鯖を動かすうえで絶対に必要な部分だけを解説します。
---

::Alert
この記事は一部内容が古い可能性があります。（特にCloudflare tunnel周りは常にドキュメントを読んで最新の情報を元に構築してください。）
::
...
::Alert
管理者アカウント作成前にこの画面をSNS等に投稿しないでください。初期設定用パスワードを設定していない状態では、他人に管理者アカウントを勝手に作成されるリスクがあります。
::
...
### オブジェクトストレージ/バックアップの設定（2026/01/18追記）

オブジェクトストレージの設定方法（Cloudflare R2使用）と、misskeyのデータベースのバックアップ方法をまとめた記事を投稿しました。合わせて設定しておくことを強くおすすめします。

[記事: MisskeyのDBバックアップとオブジェクトストレージの設定方法](https://www.bokukoha.dev/blog/misskey-setting/)
